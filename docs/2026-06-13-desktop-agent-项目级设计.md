...
> **v3 核对（2026-06-14，已对照代码复核）**：项目级功能已落地，以下设计点已被实现调整，详见 `2026-06-14-desktop-agent-项目级实现核对.md`：
> - **§0决策1 realpath 已落地**：主进程用 `fs.realpath` 解析 dirPath 作项目身份（非仅路径规范化），符合设计。
> - **§0决策2 / §3 迁移策略改了**：原设计「旧会话/旧数据清掉、不迁移」；实现改为把旧全局 trace/stats **迁移分发**到各项目桶（按会话所属项目，幂等）。
> - **§0决策2** 默认项目现绑定 `<workspace>/default` 真实目录（非 dirPath=null）。
> - **§5 删除**：后端级联清理已就绪，但 **UI 删除入口暂未实现**（侧栏/项目设置面板无删除按钮）。
> - **§6 allowlist** 实为「全局=模式／规则=会话+项目」，无全局 rules 档。
...
2. **默认项目**：始终存在一个「默认项目」——**用户不新建项目也能直接聊天**，会话落在默认项目区。新建项目两类：① **新建项目**（在 workspace 根下创建新文件夹）；② **复用已有文件夹**（指向已存在目录）。**旧会话按所属项目迁移分发**到各项目桶（实现未采用「清掉」，而是幂等迁移）。
...
> **旧数据**：升级时把旧全局 `traces` / `stats` **迁移分发**到各项目桶（按会话所属项目，幂等），种子一个默认项目。
...
- 项目右键/设置：重命名、改目录、改配置（**注：删除的后端级联已就绪，但 UI 入口暂未实现**；设计上删项目默认只删记录+会话，保留磁盘目录）。
