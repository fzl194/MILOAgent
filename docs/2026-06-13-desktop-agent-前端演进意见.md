...
**已完成（不再列）**：安全审批层（沙箱×审批策略×风险分级×权限规则）、工具富渲染（按类型分流 + 默认收起 + 多工具合并）、全局停止（AbortSignal + spawn kill）、**项目级管理**（项目=工作目录、effective 配置、数据按项目分桶、统计按项目筛选、侧栏项目树 + 目录缺失检测——原 Phase 2 ⑤ 已落地）。

**剩余要补的大事**：**规划与审查**（从黑盒执行到 diff 可审）。再加输入区增强（@引用/斜杠）、消息操作、扩展生态。这些是 Codex 桌面版区别于「聊天框」的核心。

> **2026-06-15 复核**：原「②项目化上下文」已随项目级 P1–P5 全量落地，从剩余项移除。本文其余未实现项（Plan/Diff、@引用/斜杠、推理等级、消息操作、虚拟化、命令面板、Skills/MCP/Hooks/Sub-agent、worktree、内建 Git 等）经代码核实**仍未实现**。
...
- **会话与项目**：~~扁平会话按日期分组、无工作目录概念~~ → **项目层 + 工作目录（cwd）已落地**（项目=工作目录，会话按项目归属，侧栏项目树）。剩余缺口：无搜索、无 /fork /compact。
...
**⑤ 项目 > 线程结构 + 工作目录（cwd）** ✅ **已落地（2026-06-14，项目级 P1–P5）**
- 已实现：「项目 = 工作目录」，会话按项目归属；effective 配置（全局←项目）；trace/stats 按项目分桶；统计按项目筛选；侧栏项目树 + 目录缺失检测。详见《功能盘点 2026-06-14》§3.4。
- 尚未做：「多线程/子会话」（一个项目下多分支线程）、会话搜索、/fork /compact。
...
- **2026-06-15 复核**：**有意不做自动读 AGENTS.md/MILO.md**——改用「项目级 systemPrompt」承载项目记忆（见项目级设计 §0决策6）；文件树侧栏仍未实现。
...
| 项目/线程 + 工作目录 | 🔴 高（解锁真实用途） | 中 | ✅ **已落地**（项目级 P1–P5；多线程/搜索仍未做） |
...
Phase 1  协作（差异化核心）
  Plan 模式 + Diff 审查 → @引用 + 斜杠命令 → 消息操作 + 虚拟化 → 推理等级
        ↓ 从聊天框变工作台
Phase 2  项目化（核心已落地 ✅）
  ✅ 项目>线程 + 工作目录（已落地）→ AGENTS.md（有意不做，改用 systemPrompt）+ 文件树（未做）→ 并行/后台 + Git（未做）
        ↓ 解锁真实项目用途（已部分解锁）
Phase 3  生态
...
| 问题 | 答案 |
|---|---|
| **已完成** | 安全审批层 + 工具富渲染 + 全局停止（原 Phase 1）+ **项目级管理**（原 Phase 2 ⑤） |
| **下一个差异化关键** | Plan 模式 + Diff 审查面板（Codex 同款核心） |
| **解锁真实用途** | ✅ 项目/工作目录已落地；剩余 AGENTS.md（改用 systemPrompt）+ 并行/后台 + Git |
