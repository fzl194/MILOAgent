...
> **演进 vs 现状核对(2026-06-16):P0 接缝落地(默认未激活)/ P1 未启动 / P2·P3 部分。**
> - **P0(接缝完成、激活待 P1)**:默认 agent 身份基提示文案(角色/工具规范/安全口径)与「前缀+后缀」两段式装配抽象都已落地,关闭路径逐字节等价于旧的「返回单串」(行为保持达成);**但身份开关默认仍关**(代码自述等 P1 翻开),故 effective systemPrompt 事实上仍可能为空——文中「systemPrompt 不再默认空」这一目标**尚未真正达成**,目前是「接缝就绪、开关在 off」。
> - **P1(未启动)**:无 CLAUDE.md/AGENTS.md 自动发现加载、无当前日期注入、无工作区状态注入(文档强调 git 状态属每轮变动、绝不能进前缀——目前根本未注入,无违规但也无收益)。
> - **P2(部分)**:静态/动态分段与「身份段字节稳定」的设计意图已落进代码;但 cached_tokens 虽已采集却未做命中率前后对比,ModelConfig 也**尚未加** supportsPromptCache 缝。
> - **P3(部分)**:工具结果折叠已有「已折叠」占位告知;完整 FRC 截断告知段、cache-aware 裁剪**均未实现**(仅留策略接缝)。
> - **一句话**:组织层目前只立了「分层装配」的骨架与一份默认身份文案;记忆/日期/缓存优化这条主线尚未推进。下阶段最该做的是翻开身份开关 + 落地记忆/日期注入(P1)。
